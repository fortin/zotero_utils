...
csv_log_path = pdf_dir / "zotero_link_log.csv"
...
for entry in bib_db.entries:
    ...
    try:
        ...
        continue
    if not items:
        log_rows.append({"CitationKey": citation_key, "File": "", "Result": "❌ Item not found in Zotero"})
        continue
    ...
    if not pdf_path or not pdf_path.exists():
        log_rows.append({"CitationKey": citation_key, "File": "", "Result": "❌ No matching PDF found"})
        continue
    ...
    r = requests.post(f"https://api.zotero.org/users/{ZOTERO_USER_ID}/items/{item_key}/attachments",
                      headers=headers,
                      json={
                          "itemType": "attachment",
                          "linkMode": 2,
